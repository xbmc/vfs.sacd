                             ...
                             std::vector<AudioEngineChannel>& channellist)
{
  ...
  if (toLoad.find(".sacdstream") != std::string::npos)
  {
    ...
    toLoad = filename.substr(0, slash);
  }
  ...
  int rv = m_dsdPCMDecoder->init(m_pcmOutChannels, m_framerate, m_dsdSamplerate, m_pcmOutSamplerate,
  ...
  if (rv < 0)
  {
    ...
    int rv =
        m_dsdPCMDecoder->init(m_pcmOutChannels, m_framerate, m_dsdSamplerate, m_pcmOutSamplerate,
                              conv_type_e::DSDPCM_CONV_DIRECT, CSACDSettings::GetInstance().GetConverterFp64(), nullptr, 0);
    if (rv < 0)
    {
      return false;
    }
