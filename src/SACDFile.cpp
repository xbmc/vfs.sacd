...
extern "C"
{
  ...
  struct SACDContext : public sacd_core_t
  {
...
{
  fprintf(stderr, "---------------------> %s %s\n", __PRETTY_FUNCTION__, url.GetHostname().c_str());
  std::string file(url.GetFilename());
  int track = strtol(file.substr(0, file.size() - 4).c_str(), 0, 10);

  SACDContext* sacdContext = new SACDContext;
  bool ret = sacdContext->open(url.GetHostname());
  if (!ret)
  {
    delete sacdContext;
    return nullptr;
                              ...
                              std::string& rootPath)
{
  ...
  if (strncmp(url.GetURL().c_str(), "sacd://", 7) == 0 && !url.GetHostname().empty())
  {
    ...
    ret = sacdContext.open(url.GetHostname());
  }
  else
  {
    encoded = URLEncode(url.GetURL());
    ret = sacdContext.open(url.GetURL());
  }

  uint32_t track_count = GetSubsongCount(sacdContext);
  if (track_count > 0)
  {
    ...
    for (uint32_t i = 0; i < track_count; ++i)
    {
      ...
      uint32_t subSong = GetSubsong(sacdContext, i);
      sacdContext.sacd_reader->get_info(subSong, info);

      item.SetLabel(info.GetTitle());
...
uint32_t CSACDFile::GetSubsongCount(SACDContext& sacdContext)
{
  uint32_t track_count = 0;
//   switch (CSACDPreferences::get_area())
//   {
//     case AREA_TWOCH:
//       track_count = sacdContext.sacd_reader->get_track_count(AREA_TWOCH);
//       if (track_count == 0)
//       {
//         sacdContext.sacd_reader->set_mode(sacdContext.access_mode | AREA_MULCH);
//         track_count = sacdContext.sacd_reader->get_track_count(AREA_MULCH);
//       }
//       break;
//     case AREA_MULCH:
//       track_count = sacdContext.sacd_reader->get_track_count(AREA_MULCH);
//       if (track_count == 0)
//       {
//         sacdContext.sacd_reader->set_mode(sacdContext.access_mode | AREA_TWOCH);
//         track_count = sacdContext.sacd_reader->get_track_count(AREA_TWOCH);
//       }
//       break;
//     default:
//       track_count = sacdContext.sacd_reader->get_track_count(AREA_TWOCH) +
//                     sacdContext.sacd_reader->get_track_count(AREA_MULCH);
//       break;
//   }
...
uint32_t CSACDFile::GetSubsong(SACDContext& sacdContext, uint32_t p_index)
{
  return sacdContext.sacd_reader->get_track_number(p_index);
}
